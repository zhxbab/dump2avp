...
uncore_prolog_start = 0
uncore_prolog_end = 0
with open("uncore_prolog","r") as fd:
    while True:
        line = fd.readline()
        if line:
            line = line.strip()
            if re.search(r'uncore start',line):
                uncore_prolog_start = 1
            if re.search(r'uncore end',line):
                uncore_prolog_end = 1
            if uncore_prolog_start == 1 and uncore_prolog_end == 0:
                m = re.search(r'// (\w+) \[(\d+)\] - (\w+)', line)
                if m:
                    if eq("WB",m.group(1)) or eq("Load 4",m.group(1)) :
                          pass
                    else:
                        print("name: %s_%s_%s"%(m.group(1),m.group(2),m.group(3)))
...
#                last_sig_name = m.group(1)
                ...
                if m:
                    print("position: %x and value: %s"%(int((int(m.group(1),16)-0x78-0x6C8-0x530-0x108)/4),m.group(2)))
