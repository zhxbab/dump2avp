...
value = ""
p_flag = 0
with open("core_prolog_log","r") as fd:
    while True:
        line = fd.readline()
        if line:
            line = line.strip()
            ...
            if m:
                ...
                if m:
                    if p_flag and not re.search(r'xmm\d+',sig_name_base) and not re.search(r'ymm\d+',sig_name_base) and not re.search(r'st\d+',sig_name_base) :
